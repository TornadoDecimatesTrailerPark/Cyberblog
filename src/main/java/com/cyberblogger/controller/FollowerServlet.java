...
@WebServlet(name = "FollowerServlet", urlPatterns = {"/follower"})
...
//            User user = FollowerDao.getUserInfoById(Integer.parseInt(userId), DBConnectionUtils.getConnectionFromClasspath("connection.properties"));
            ArrayList<Follower> followers = FollowerDao.getFollowerByUserId(Integer.parseInt(userId), DBConnectionUtils.getConnectionFromClasspath("connection.properties"));
//            request.setAttribute("user", user);
            request.setAttribute("followers", followers);
        } catch (SQLException | NumberFormatException e) {
            e.printStackTrace();
        }
        request.getRequestDispatcher("WEB-INF/view/follower.jsp").forward(request, response);
    }
