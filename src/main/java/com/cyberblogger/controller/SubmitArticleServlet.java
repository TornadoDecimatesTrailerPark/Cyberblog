...
@WebServlet(name = "SubmitArticleServlet", urlPatterns = {"/submitArticle"})
public class SubmitArticleServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      try {
        if (article.getId() == 0){
          //insert article
          if(ArticleService.addNewArticleAllInfo(article)){
            result.setData(article.getId());
          }else {
            result.setMessage("submitting new article failed");
            result.setCode(-1);
          }
        }else {
          //edit article update
          if (!ArticleService.updateArticleAllInfo(article)){
            result.setMessage("updating new article failed");
            result.setCode(-1);
          }
        }
